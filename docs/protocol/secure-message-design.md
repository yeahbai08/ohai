...
- **设备能力模型与 Schema 格式**：详见 [设备能力模型与 Schema 设计](./capability-model.md)
...
在 [设备能力模型](./capability-model.md#_3-命令与状态的结构化关联) 中，`state_cmd` 必须通过 `affects` 字段显式声明它影响的状态，这在 Schema 层面将命令与状态关联起来，引导开发者做出正确的设计决策。
:::
...
Schema（`schema.json`）定义了设备的能力、命令和事件，是设备与 AI 引擎交互的基础。Schema 的能力模型和文件格式详见 [设备能力模型与 Schema 设计](./capability-model.md)。

为了防止恶意设备注册虚假 Schema 进行提示词注入攻击，OHAI 使用 OVR（开放厂商注册表）验证 Schema 的完整性和来源：厂商将 `schema.json` 文件及其 SHA-256 哈希通过域名端点发布并在 OVR 注册，Server 在设备注册时将设备提供的 Schema 原文哈希与 OVR 记录比对，匹配则自动接受，不匹配则拒绝。OVR 中无记录的 Schema 进入开发者模式，由用户在 Console App 中手动审核后决定是否接受。OVR 将所有 Schema 公开托管、公众可审计，以降低提示词注入风险（详见 [Schema 公开可见性与 AI 安全](./secure-net-design.md#schema-公开可见性与-ai-安全)）。

Schema 中包含厂商名、产品名以及固件版本号。这三者唯一标识了一个 Schema（`schema_id = keccak256(vendor_name + product_name + firmware_version)`）。设备在注册时提供这三者信息以及 `device_id`，Server 根据 `schema_id` 从本地缓存查找对应的 Schema 哈希进行比对，验证通过后将 Schema 与 `device_id` 绑定，并存储在设备目录中。AI 引擎在生成命令时会根据设备别名或 `device_id` 查找对应的 Schema，确保生成的命令符合设备能力定义。
...
AI 自动化是 OHAI 的核心功能之一。完整的自动化规则模型（Trigger → Condition → Action）、状态-事件-命令协作闭环、规则示例和遥测数据存储详见 [OHAI 协议框架 - AI 自动化集成](./protocol-framework.md#_9-ai-自动化集成)。

设备厂家提供的控制面板 UI（基于 Adaptive Cards）详见 [设备能力模型 - 设备控制面板](./capability-model.md#_10-设备控制面板-adaptive-cards)。
