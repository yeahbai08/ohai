...
OHAI 在 **标准能力定义** 中为每条命令声明 AI 安全策略。该策略约束所有由 AI 引擎决策的操作——包括自动化规则触发的命令和 AI 响应用户语音/文本指令时生成的命令。该策略是能力 Schema 的组成部分，由 OHAI 标准库定义，开发者和用户均无法绕过。
...
当同一命令的不同参数值具有不同风险等级时，使用 `ai_policy_by_params` 进行 **参数级策略声明**。`when` 使用 JSON Schema 子集语法匹配参数值，按声明顺序首条命中生效。**未命中任何 `when` 时回退至 `confirm`**——这确保未被显式覆盖的参数组合默认需要用户确认，防止因遗漏 `when` 条件而意外放行危险操作。若开发者希望对未匹配参数使用其他策略，应添加一条无条件的兜底 `when`（匹配所有参数）。

**三层单向升级模型**：

`ai_policy` 的最终生效策略由三层叠加决定，每层只能升级不能降级：

| 层级 | 定义方 | 说明 |
|---|---|---|
| **标准能力定义** | OHAI 标准库 | 安全下限，定义每条命令的最低安全等级 |
| **厂商 Schema 覆盖** | 设备固件 | 通过 `overrides` 将策略升级（`allow → confirm → deny`），不允许降级 |
| **用户设备配置** | Console App | 用户在运行时可进一步升级策略，不允许降级 |
