...
**原因**：设备消息最终由 AI 引擎读取和处理。若允许设备发送任意字符串，恶意或被攻破的设备可通过状态上报、事件参数或命令回复向 AI 引擎注入提示词攻击（Prompt Injection）。封闭枚举确保 AI 引擎仅接收预定义的安全标识符。

**规则**：
- States 中的 `type: string` 必须声明 `enum`
- Command `params` 和 `result` 中的 `type: string` 必须声明 `enum`
- Event `params` 中的 `type: string` 必须声明 `enum`
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `brightness` | integer | 0-100, unit: % | 亮度百分比 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `color_temp` | integer | 2700-6500, unit: K | 色温 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `target_humidity` | integer | 0-100, unit: % | 目标湿度 |
...
| `filter_life` | integer | 0-100, unit: % | 滤网剩余寿命 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `mode` | string | enum: disarmed, armed_home, armed_away, armed_night | 布防模式 |
| `triggered` | boolean | — | 是否已触发报警 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `position` | integer | 0-100, unit: %（0=完全关闭, 100=完全打开） | 门位置 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `playback_state` | string | enum: playing, paused, stopped, idle | 播放状态 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `volume` | integer | 0-100, unit: % | 音量百分比 |
| `muted` | boolean | — | 是否静音 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `power` | number | unit: W | 当前功率 |
| `voltage` | number | unit: V | 当前电压 |
| `current` | number | unit: A | 当前电流 |
...
| States | 类型 | 约束 |
|---|---|---|
| `battery_level` | integer | 0-100, unit: % |
...
|---|---|---|
| `aqi` | integer | 0-500 |
| `level` | string | enum: good, moderate, unhealthy_sensitive, unhealthy, very_unhealthy, hazardous |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `status` | string | enum: idle, cleaning, paused, returning, charging, docked, error | 工作状态 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `status` | string | enum: idle, running, paused, complete, error | 工作状态 |
...
| 键名 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `status` | string | enum: idle, running, paused, cooling, complete, error | 工作状态 |
