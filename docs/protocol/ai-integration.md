...
本文档定义 OHAI 的 AI 集成机制：Schema 到 LLM Tool Calling 的自动映射，以及 AI 自动化规则运行模型。设备控制面板规范已拆分至 [设备控制面板](./device-panel-ui.md)。
...
| 定时器 | `timer_active?(name)` | 查询命名定时器是否仍在等待触发 |
...
1. **澄清边界条件**：用户很难一句话描述清楚需求。LLM 主动询问未明确的细节——触发阈值、时间窗口、异常情况处理、是否需要通知等，与用户逐步确认一份完整的需求描述。LLM 应主动引导用户考虑**迟滞区间**（hysteresis）——例如"温度超过 30°C 开空调"应配合"温度降到 27°C 以下才关空调"，而非在 30°C 阈值上反复开关
...
- **规则间关系**：标注与其他规则的关联——共享设备、所属互斥组、组内优先级排序
- **执行历史**：展示规则近期的执行记录和抑制记录，包括"为什么这条规则没有执行"的原因链（详见[执行可解释性](#执行可解释性)）
...
当用户**显式开启“基于历史数据的智能建议”**后，OHAI Server 才会启动周期性分析任务。分析任务不会默认抓取全屋历史，而是先按场景构建设备白名单、时间窗和特征摘要，再将最小必要输入交给模型进行模式识别。默认优先使用本地模型；若启用云模型，必须额外获得单独授权。用户确认后，LLM 再将建议转化为具体的规则代码并部署。这个过程不是一条预编写的规则，而是系统的内在智能。

历史分析的默认策略、数据分级、审计与删除要求，详见 [历史分析数据保护规范](./history-analysis-data-protection.md)。

**场景**：用户已在 Console App 中开启“历史分析建议”，并仅允许“热水设备”和“相关流量传感器”参与分析。系统使用本地模型，对最近 14 天的热水器 `water_heater_01` 运行数据进行例行分析。分析任务不会上传全量原始序列，而是先由 Server 提取候选特征，例如用水开始时间分布、加热持续时长和工作日/周末差异。LLM 在例行分析中发现了如下模式——
...
系统随即生成一条**建议依据摘要**并通过 Console App 向用户推送建议：

> **建议依据摘要**：
> - 分析时间窗：最近 14 天
> - 参与设备类别：热水器、水流量传感器
> - 模式摘要：12/14 天在 6:50–7:10 之间出现持续 15–25 分钟的用水与加热
> - 置信度：0.91
> - 模型位置：本地模型
...
用户查看建议依据摘要和模拟结果后确认接受。LLM 生成以下规则并部署到 Server：
...
**要点**：这个示例展示了 OHAI 与前几个示例根本不同的一面——规则代码本身并不复杂，真正的智能在于**发现用户习惯并主动建议**的过程。但这个过程必须受治理约束：只有在用户显式开启后，系统才启动历史分析；分析输入需要先经过设备白名单、时间窗和粒度裁剪；默认优先使用本地模型；每条建议都要附带依据摘要并可追溯到分析任务。用户确认后，LLM 才生成规则代码。这种“显式授权 → 最小化分析 → 洞察 → 建议 → 确认 → 部署”的闭环，才能让 OHAI 在保持智能性的同时具备可接受的隐私与安全边界。
...
互斥组是一组**竞争同一物理控制目标**的规则。同一互斥组内的规则不能同时生效——当多条规则同时触发时，只有优先级最高的规则执行，其余规则被抑制。

互斥组的分组维度是 **`(device_id, capability)`**——即同一设备的同一能力。这是一个自然的物理边界：向同一设备的同一能力下发命令的规则，本质上是在竞争对同一个物理状态的控制权。

**示例**：以下三条规则都向 `(ac_456, ohai.thermostat)` 下发命令，自动归入同一互斥组：

| 规则 | 意图 | 命令目标 |
|---|---|---|
| 高温制冷 | 温度 >30°C 时开空调制冷 | `ac_456 / ohai.thermostat` |
| 窗户联动 | 窗户打开时关空调 | `ac_456 / ohai.thermostat` |
| 节能限制 | 电价高峰时限制空调温度 | `ac_456 / ohai.thermostat` |

三条规则单独看都合理，但如果没有互斥组协调，温度传感器上报高温 → 高温制冷开空调 → 节能限制立刻调高目标温度 → 温度再次升高 → 高温制冷再次调低 → 形成振荡。互斥组确保同一时刻只有优先级最高的规则生效。

**如何识别互斥组？**

1. **自动识别（主要方式）**：Server 对所有规则代码进行静态分析，提取每条规则的 `API.send_command` 调用目标。共享同一 `(device_id, capability)` 目标的规则自动归入同一互斥组。一条规则如果控制多个设备/能力，会同时属于多个互斥组
2. **LLM 辅助识别（跨设备冲突）**：有些物理冲突跨越不同设备——例如"开空调"和"开窗通风"分别控制空调和窗户，命令目标不同但物理效果矛盾。LLM 在规则创建的需求分析阶段识别这类跨设备冲突后，建议用户建立跨设备互斥关系
3. **用户调整**：用户可以在 Console App 中手动调整互斥关系——合并组、拆分组、将规则移入或移出某个组

**互斥组的运行时行为**

当同一互斥组内的多条规则同时触发：

1. 按优先级排序，最高优先级的规则执行命令
2. 低优先级规则的命令被**抑制**（不下发），并记录抑制日志
3. 互斥组进入**冷却期**（cooldown，默认可配置，如 5 分钟）——冷却期内，同组其他规则的命令被暂缓，防止低优先级规则在高优先级规则刚执行后立刻反转设备状态
4. 冷却期结束后恢复正常的优先级仲裁

冷却期的意义：没有冷却期时，高温制冷执行完毕后的下一秒，节能限制可能立刻触发并改变温度设置。冷却期给高优先级规则的效果一个"保持窗口"，避免被低优先级规则立刻覆盖。

**Console App 中的互斥组管理**

- **规则详情页**：每条规则展示其所属的互斥组，以及组内其他规则和自己的优先级排序
- **互斥组视图**：专门的管理页面列出所有互斥组，每组显示成员规则、优先级顺序、冷却时间设置
- **可调整项**：组内规则优先级、冷却时间、添加/移除规则
- **抑制记录**：每组显示近期的抑制事件——哪些规则被抑制、原因是什么、它们本来要做什么

#### 创建时冲突分析

在规则创建阶段，Server 对新规则进行多层次冲突分析：

1. **命令级冲突**：检测新规则是否会向同一设备的同一能力下发与现有规则矛盾的命令（例如一条规则要开空调制冷，另一条要开制热），并自动将其归入对应的互斥组
2. **设备级冲突**：检测多条规则是否在相近时间窗口内操作同一设备的不同能力，可能导致设备状态不一致
3. **目标级冲突**：LLM 分析规则的意图是否与现有规则存在目标竞争——例如一条追求舒适、一条追求节能，虽然命令不直接矛盾但可能导致系统行为拉扯。对于跨设备的目标冲突，LLM 建议建立跨设备互斥关系
...
1. **因果链追踪**：防止规则直接或间接触发自身（详见下文）
2. **互斥组仲裁**：同一互斥组内多规则同时触发时，按优先级仲裁，低优先级命令被抑制并进入冷却期
3. **矛盾命令检测**：`API.send_command` 在执行前检查是否有其他规则在短时间窗口内向同一设备的同一能力下发了矛盾命令，作为互斥组之外的兜底检测
4. **振荡检测**：Server 持续监控每个设备的命令历史，当检测到同一设备在短时间内被多条规则反复操作（如灯被反复开关、空调在制冷和制热之间切换）时，自动暂停相关规则并通知用户。振荡判定标准：同一设备同一能力在可配置的时间窗口内收到超过阈值次数的状态翻转命令

#### 因果链追踪

**问题**

规则调用 `send_command` 控制设备后，设备状态变化会产生新事件，这些事件可能再次触发同一规则（直接自触发）或经过其他规则的级联后最终触发回自身（间接自触发）：

- **直接自触发**：规则 A 开空调 → 空调状态变为"制冷" → 触发规则 A 的 `on_state_change` → 规则 A 再次执行
- **间接自触发**：规则 A 控制设备 X → 触发规则 B → 规则 B 控制设备 Y → 触发规则 A

**为什么不用 Home Assistant 的 Automation Mode 方案？**

Home Assistant 为每条自动化设置 `mode`（single/restart/queued/parallel），控制同一规则被重复触发时的行为。这是一个基于时序的粗粒度锁，存在两个问题：

- **误杀合法事件**：`mode: single` 在规则运行期间阻止所有重复触发，包括与当前执行无因果关系的独立事件（如用户恰好在规则运行时手动触发了传感器）
- **无法覆盖间接循环**：A → B → A 的间接自触发完全无法检测

**OHAI 的方案：因果上下文传播**

OHAI Server 控制从 `send_command` 到事件分发的完整链路，天然知道每一个状态变化是由谁引起的。Server 利用这一优势进行因果链追踪：

1. 每次规则执行时，Server 为其分配一个**因果上下文**（Causality Context），包含触发来源和已经过的规则链
2. 规则调用 `send_command` 时，命令携带该因果上下文
3. 命令导致的设备状态变化事件**继承**同一因果上下文
4. 事件分发到规则前，Server 检查目标规则是否已在因果链上——如果是，说明这是自触发循环，**抑制该次触发**

```
规则 A 执行 send_command("ac_456", ...)
  → 命令携带因果上下文 {origin: rule_a, chain: [rule_a]}
    → 设备响应 → Shadow 更新 → 状态变化事件继承同一上下文
      → 事件分发到规则 A 的 on_state_change
        → Server 检查：rule_a 已在 chain 中 → 自触发，抑制
      → 事件分发到规则 B 的 on_state_change
        → Server 检查：rule_b 不在 chain 中 → 正常触发
          → 规则 B 执行 send_command("light_123", ...)
            → 命令携带 {origin: rule_b, chain: [rule_a, rule_b]}
              → 状态变化事件分发到规则 A
                → Server 检查：rule_a 在 chain 中 → 间接自触发，抑制
```

**默认行为与安全边界**

- **默认抑制所有自触发**：直接和间接自触发一律抑制并记录日志。这对绝大多数场景是正确的——规则开了空调，不应再被"空调已开启"的状态变化重新触发
- **因果链深度上限**：即使不存在循环，因果链也设置最大深度（默认 10），防止无环但过长的级联触发链消耗过多资源。达到深度上限时，后续触发被抑制并记录警告日志
- **外部事件不受影响**：用户手动操作设备、设备自身上报的传感器数据等外部事件不携带因果上下文，始终正常触发规则

**与其他方案的对比**

| 场景 | Home Assistant（mode: single） | SmartThings（trigger: Never） | OHAI（因果链） |
|---|---|---|---|
| A 开空调 → 空调状态变化 → 再次触发 A | A 还在运行时阻止，否则放行（漏放） | 需手动标记条件（用户负担） | 精确识别，抑制 |
| A 运行中，用户手动触发同一传感器 | 阻止（误杀） | 不受影响 | 不受影响 |
| A → B → A 间接循环 | 无法检测 | 无法检测 | 因果链追踪，精确抑制 |
| 级联深度过大（A → B → C → D → ...） | 无保护 | 无保护 | 深度上限截断 |

#### 执行可解释性

当规则未执行或被抑制时，系统记录完整的原因链，用户可在 Console App 中查看每条规则的执行历史，包括"为什么这条规则没有执行"：

| 抑制原因 | 说明 |
|---|---|
| 自触发（直接） | 规则自身的 `send_command` 导致的状态变化触发了自身 |
| 自触发（间接） | 因果链：规则 A → 规则 B → ... → 回到规则 A |
| 因果链深度超限 | 级联触发链已达最大深度（默认 10） |
| 互斥组仲裁 | 同组的规则 X（优先级更高）已执行，本规则被抑制 |
| 冷却期 | 同组的规则 X 在 N 秒前刚执行，冷却期未结束 |
| 矛盾命令 | 规则 X 在短窗口内向同一设备下发了矛盾命令 |
| `ai_policy` 拦截 | 命令被安全策略拦截（`deny`）或等待用户确认（`confirm`） |
| 触发条件未满足 | 事件到达但 `when` guard 条件不成立 |

每条抑制记录同时保存"规则本来要做什么"——即被抑制的命令内容和参数，方便用户判断抑制是否合理。
